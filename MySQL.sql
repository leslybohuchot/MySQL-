...
SELECT first_name, last_name, payment_date, amount, staff_id
FROM staff INNER JOIN payment 
ON staff.staff_id = payment. starr_id AND payment_date LIKE '2005 -08'
...
SELECT title, COUNT(rental_id) as 'Final Rental'
FROM rental
...
INNER JOIN inventory ON film_category_id = inventory.film_id 
INNER JOIN rental ON inventory.inventory_id = rental.inventory_id
INNER JOIN payment ON rental.rental_id = payment.rental_id
GROUP BY name
ORDER BY sum(amount);
-- 8a. In your new role as an executive, you would like to have an easy way of viewing the Top five genres by gross revenue. Use the solution from the problem above to create a view. If you haven't solved 7h, you can substitute another query to create a view.
CREATE VIEW revenues_by_genre AS 
SELECT name, SUM (amount)
FROM category
INNER JOIN film_category ON category.category_id = film_category.category_id
...
SELECT * FROM revenues_by_genre;
-- 8c. You find that you no longer need the view top_five_genres. Write a query to delete it.
DROP VIEW revenues_by_genre
